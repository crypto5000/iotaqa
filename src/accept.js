$(document).ready(function() {
    ...
    try{
        ...
        bountyAddress = localStorage.getItem("bountyAddress");
        viewAddress = localStorage.getItem("qAddress");                    

        // clear the localstorage for future use        
        window.localStorage.clear();

        // check if field has a answer
        if (!answer) {
            alert("Answer is invalid. Please enter a value.");                                        
            return false;
        }

        // validate the answer length
        if (answer.length > 750) {
            alert("Answer is invalid. Answer should be less than 750 characters.");                                        
            return false;
        }

        // check if field has a bounty address
        if (!bountyAddress) {
            alert("Bounty address is invalid. Please enter a value.");                                        
            return false;
        }

        // validate the address length             
        if (bountyAddress.length !== 90) {
            alert("Bounty address is invalid. Addresses should be 90 characters.");                                        
            return false;
        }

        // validate the address content
        for (var j = 0; j < bountyAddress.length; j++) {
            if (("9ABCDEFGHIJKLMNOPQRSTUVWXYZ").indexOf(bountyAddress.charAt(j)) < 0) {                            
                alert("Bounty address should contain only letters A-Z or the number 9. Address is invalid.");                                        
                return false;
            }
        }

        // check if field has a address
        if (!viewAddress) {
            alert("Question address is invalid. Please enter a value.");                                        
            return false;
        }

        // validate the address length             
        if (viewAddress.length !== 90) {
            alert("Question address is invalid. Addresses should be 90 characters.");                                        
            return false;
        }
        ...
        for (var j = 0; j < viewAddress.length; j++) {
            if (("9ABCDEFGHIJKLMNOPQRSTUVWXYZ").indexOf(viewAddress.charAt(j)) < 0) {                            
                alert("Question address should contain only letters A-Z or the number 9. Address is invalid.");                                        
                return false;
            }
        }

        // start the send process
        acceptAnswer(answer,viewAddress,bountyAddress);

    } catch(err){
        console.log("curl not supported");            
        alert("Webgl2 browser required to ask question. Try desktop Firefox or Chrome.");
        return false;
    }

    function acceptAnswer(answer,viewAddress,bountyAddress) {
        ...
        let value = 0;             
        let message = bountyAddress + "" + answer;
        let tag = "IOTAQNACCCCCCCCCCCCCCCCCCCA";                       
        let depth = 10;
        let weight = 14;
        ...
        const localAttachToTangle = function(trunkTransaction, branchTransaction, minWeightMagnitude, trytes, callback) {
            ...
            $('#loadingText').html("Attaching Acceptance to Tangle");		
        ...
        function sendMessages(viewAddress) {
            ...
            iota.api.sendTransfer(seed, generateDepth(), weight, transfers, function(error, success){
                ...
                $('#loading').hide();
                $('#loadingText').html("Success. Acceptance is Now In the Tangle.");	
                $('#breakGap').hide();	
                $('#displaySuccess').show();	
